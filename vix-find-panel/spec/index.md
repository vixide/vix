...
**workspace-wide search & replace** (`Ctrl+Shift+F` or palette "Search in Workspace" /
"Search and Replace in Workspace"; searches open buffers in their unsaved state), and
...
All replace capability lives **inside the Find panel** — there is no separate
"Find & Replace" menu item. Open the find box with `Ctrl+F`, then reveal the
Replace field with `Ctrl+R` or by pressing `Tab`; everything from a single
replacement to Replace All, interactive query-replace, and workspace-wide replace
is driven from here (and the matching palette commands).
...
Open with `Ctrl+Shift+F`, or "Find in Workspace" / "Find and Replace in Workspace"
from the command palette. Type to search incrementally across every file under
the workspace root; results list as `path:line: text`. Use ↑/↓ to navigate and
...
across the workspace. Open buffers are searched and replaced in their current
...
against each file's workspace-relative path (forward-slashed): **Include path**
...
Find → Find in Workspace → Dock** (or the palette). It prompts for a term —
...
**click-to-jump** to the match. See `vix-bottom-dock/spec/index.md`.

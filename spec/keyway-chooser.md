...
Exactly one keyway is active at a time.

The user can choose which keyway is active by using the "View" menu -> "Keyway..." menu item.

## As implemented in Vix

**Status:** Shipped. The chooser (`vix-keyway-chooser`) and a working subset of
each keyway's bindings are implemented; the choice persists in the `keyway`
...
function keys (`F1`, `F3`, `F10`, `F12`) work in every keyway.

| Keyway | What Vix does today |
| ------ | ------------------- |

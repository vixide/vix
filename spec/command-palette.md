...
- In go-to-line mode (:) and in file-finder mode, append `:<N>` (optionally `:<col>`) to jump to that position; the jump commits when you press Enter. (A live cursor preview while typing the number is roadmap — see the status note above.)

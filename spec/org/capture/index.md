...
Every one of these is a _fixed, single-purpose_ prompt: one hardcoded shape,
...
- Any number of _named templates_, each with a key, a description, a target
...
A template engine turns that hardcoded function into data any user can
...
| ------------------- | ------------------------------------------------- | ------------------------------------------------ |
| `entry` (default)   | A headline: `* <expanded template>`               | `org.capture`, `org.capture.task`                |
| `plain`             | Raw text, no headline wrapper                     | `org.capture.babel` (its `"b"` template)         |
...
**Capture → Anything… / Babel… / Contact… / Note… / Task…** (Anything first
as the quick default, then the rest alphabetically) run the built-in
`"a"`/`"b"`/`"c"`/`"n"`/`"t"` templates by key (`App::start_capture_by_key`)
via actions `org.capture`/`org.capture.babel`/`org.capture.contact`/
`org.capture.note`/`org.capture.task`; **Capture → Choose Template…**
(`org.capture.select`) opens every configured template in a chooser
(`CaptureChooser`), arrow-key/click selectable — this is where custom
templates beyond the five built-ins are reached, since `vix-menu`'s `Item`
tree is `&'static` (compile-time) and can't be populated from
`org_capture_templates` at runtime. Selecting a
template (`App::start_capture`):
...
7. **`org.capture.*` action namespace.** `org.capture_todo` → `org.capture.task`
   and `org.contacts.new` → `org.capture.contact`, so every built-in capture
   action sits under one `org.capture.*` prefix (matching the menu item ids,
...
8. **`org.capture.note` (a fifth built-in).** `"n"` template
   (`"* %^{Note}\n  %U"`), the classic Emacs org-capture "Note" shape: a
   plain headline (no `TODO` keyword, unlike `"a"` Anything) plus a `%U`
   inactive creation timestamp. `immediate_finish = true`, matching
   Anything/Contact — a single prompt, then files right away.
9. **Live template preview on field prompts.** The original wizard showed
   ...
   `‹Label›`, later ones `[Label]`, and every non-field placeholder expanded
   immediately (matching Emacs, where `%t`/`%a`/etc. resolve the instant the
   capture buffer opens, before any minibuffer prompt). `draw_prompt`
   (`src/ui.rs`) renders it above the input, scrolled to keep the current
   field's line in view.

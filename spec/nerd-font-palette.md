...
opens it from the **Tools** menu → **Nerd Font Palette…**, browses a grid of icon
...
- **Enter** inserts the highlighted glyph at the editor cursor and **keeps the
  palette open**, so several glyphs can be picked in a row.
...
`vix-nerd-font-palette` crate (pure data, no dependencies). The host (`src/app.rs`,

...
loop, `App` state, rendering, and the explorer — and it depends on the 119
...
| AI          | `vix-ai-panel` (chat panel), `vix-ai-diff` (AI diff review), `vix-ai-core` (T124: direct HTTP clients for Anthropic/OpenAI-compatible/Ollama, an alternative to the CLI shell-out every AI feature still defaults to). |
...
| Path            | Contents                                                            |
| --------------- | ------------------------------------------------------------------- |
| `crates/`       | The 119 `vix-*` workspace member crates (each with its own `spec/`).|

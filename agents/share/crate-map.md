...
| Keybindings | `vix-keybindings` — an exhaustive, queryable registry of every built-in keybinding across all 10 keymap ids, plus the user (`keybindings.toml`)/script (`bind_key`) override layer built on it (`crates/vix-keybindings/spec/index.md`). **Emacs (T104a), Vi/Spacemacs (T104b), VS Code (T104c), `IntelliJ` (T104d), and Eclipse (T104e) converted**: `emacs_key`, `vim_normal_key`, `spacemacs_leader_lookup`, `vscode_ctrl_key`, `intellij_key`, and `eclipse_key` all dispatch through the registry now (`Binding`/`ChordContext`/`KeymapTable`/`lookup`/`shortcuts_for`, plus `lookup_sequence` for Spacemacs's leader-style sequence matching); `intellij-macos`/`intellij-windows` are two genuinely different tables (unlike VS Code's one shared table) — the platforms' "go to" bindings differ entirely; Eclipse's one non-`Ctrl` binding (`Alt+/`, word completion) lives alongside its `Ctrl` bindings in the same flat table. The other 2 keymap ids (Apple, Sublime) are still empty tables, one per follow-on task (T104f–g), before the persisted override file and `on_key` choke point (T104h–j). |

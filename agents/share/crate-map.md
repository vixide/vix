...
| Keybindings | `vix-keybindings` — an exhaustive, queryable registry of every built-in keybinding across all 10 keymap ids, plus the user (`keybindings.toml`)/script (`bind_key`) override layer built on it (`crates/vix-keybindings/spec/index.md`). **Emacs (T104a), Vi/Spacemacs (T104b), VS Code (T104c), `IntelliJ` (T104d), Eclipse (T104e), and Sublime Text (T104f) converted**: `emacs_key`, `vim_normal_key`, `spacemacs_leader_lookup`, `vscode_ctrl_key`, `intellij_key`, `eclipse_key`, and `sublime_key` all dispatch through the registry now (`Binding`/`ChordContext`/`KeymapTable`/`lookup`/`shortcuts_for`, plus `lookup_sequence` for Spacemacs's leader-style sequence matching); `intellij-macos`/`intellij-windows` are two genuinely different tables (unlike VS Code's one shared table) — the platforms' "go to" bindings differ entirely; Eclipse's one non-`Ctrl` binding (`Alt+/`, word completion) lives alongside its `Ctrl` bindings in the same flat table; Sublime found nothing new beyond the by-now-expected Shift-bit-explicit token need. The one remaining keymap id (Apple) is still an empty table, the last follow-on task (T104g), before the persisted override file and `on_key` choke point (T104h–j). |

# Sub Word Right

Editor action `sub-word-right`.

| Form | Identifier |
| ---- | ---------- |
| kebab | `sub-word-right` |
| snake | `sub_word_right` |
| Pascal | `SubWordRight` |

Run it from the command palette or a key binding via the action id `sub_word_right`.
It is dispatched by `App::run_action("sub_word_right")` and, for editing actions, backed
by `Editor::sub_word_right` in `editor_core`. See `crates/vix-editor-core/spec/index.md` for the full
catalog.

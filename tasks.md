...
boundable scope; the two genuinely large/different-direction ideas
(Remote/SSH editing, Collaborative editing) and the one the backlog's own
text already argued against for now (CodeQL) stay unpromoted. Sized and
...
- [x] **T553 — `vix --doctor`: a CLI subcommand (and Help menu entry)
  ...
  configured — is a skip, not a fail): `git` on `PATH` (`which`/`where`
  via a `Command::new("git").arg("--version")` spawn, cross-platform,
  same shape `spawn_ai_cli`/`run_command_in` already use); each
  configured `Settings.lsp_servers` entry's `command` actually spawns
  (`LspServer` already exists at `crates/vix-settings/src/lib.rs:469`);
  the active locale's spellcheck dictionary is discoverable
  (`vix_spellcheck::find_dictionary` against
  `vix_spellcheck::system_dictionary_dirs()`, already public API, no
  new plumbing); `TERM`/color support looks sane (`std::env::var("TERM")`
  is set and isn't `"dumb"`; note `NO_COLOR`/`COLORTERM` if set, matching
  how badge/color decisions are made elsewhere). New `--doctor` bool
  flag on `Cli` (`src/cli.rs`, alongside `--version`/`--tutor`'s own
  precedent) handled in `main.rs` before the TUI starts, printing to
  stdout and exiting — mirrors `--version`'s early-return shape
  exactly, so the man-page generator (`examples/generate_man.rs`) picks
  it up automatically once documented on the flag. Help menu entry
  runs the same checks and shows the report in a read-only overlay
  (reuse an existing text-report display, e.g. the same shape
  `Project → Compile` output or a plain message overlay uses — survey
...
- [x] **T554 — SBOM generation as a release artifact.** Emit a Software
  ...
  `.github/workflows/release.yml` is `dist`-autogenerated (a header
  comment says so explicitly) — hand-editing it directly would be
  overwritten the next time `dist generate` regenerates it from
  `dist-workspace.toml`. `dist-workspace.toml` currently has no SBOM/
  attestation config, so either (a) check whether the pinned `dist`
  version this repo uses has grown native SBOM support since
  `dist-workspace.toml` was last generated, and enable it there if so
  (the *right* place, if available), or (b) add a wholly separate,
  hand-maintained job — tag-triggered same as `release.yml`, but its
  own file, e.g. `.github/workflows/sbom.yml` — that doesn't touch
  `dist`'s generated output at all. Follow `security.yml`'s own
  established pattern for pulling in a third-party CI tool safely: a
  pinned version plus a checked SHA-256 (`CARGO_DENY_VERSION`/
  `CARGO_DENY_SHA256` right above, `.github/workflows/security.yml`),
  not a bare `cargo install`. GitHub-only is fine to start (the release
  artifacts themselves are GitHub-only already, `dist`'s own scope);
...
- [x] **T555 — Settings/profile export-import.** "Export my setup"
  ...
  (`Settings::macros_path()`), `keybindings.toml`
  (`Settings::keybindings_path()`), and the user dictionary
  (`Settings::user_dictionary_path()`) — into one archive (a plain
  `.tar.gz` or `.zip`, no new archive-format dependency needed beyond
  whatever the workspace already pulls in for something comparable —
  check first); "Import" does the reverse onto a fresh machine, or
  applies a shared team preset. All five source paths already exist as
  public `Settings` accessors (`crates/vix-settings/src/lib.rs:670-717`)
  — this task is entirely about the bundling/unbundling and the UI
  (new CLI flags, e.g. `--export-settings <path>`/`--import-settings
  <path>`, plus Settings menu entries), not new config-location
  plumbing. Decide and document import's conflict behavior explicitly
...
- [x] **T556 — A proper overlay for resolving Git merge conflicts.**
  **Correction to the Ideas-backlog framing below, found while sizing
  ...
  (`src/app/git.rs:127-131`, calling `resolve_conflict`) already accept
  or reject one side at the cursor, and a separate `conflict_next`
  action already jumps to the next conflict marker in the buffer. What
  is genuinely missing: any *overview* — a count of how many conflicts
  remain in the file (or across the whole `git status`-reported
  conflicted set), a way to jump into a specific one from a list rather
  than only linearly via "next," and an "accept both, then let me edit
  the merged text inline" path (today "both" concatenates both sides
  verbatim with no chance to reconcile them before the buffer changes
  underneath the cursor). Scope this as: a new overlay (survey
  `vix-workspace-search`'s or another existing multi-item-list panel's
  shape rather than inventing a new one) listing every conflict marker
  in the active buffer (`vix_conflict_tool::find` already walks them
  one at a time from a given offset — extend to enumerate all, or add a
  sibling `find_all`), each row showing a short preview of both sides,
  Enter jumps + closes, a keystroke per row resolves it without
  leaving the list. Workspace-wide (multiple conflicted files) is a
  natural follow-on, not necessarily this task's own scope — decide
...
- [ ] **T557 — Accessibility audit for screen readers.** T203 added a
  ...
  through a terminal's own screen-reader integration, or is every
  signal purely visual (color, cursor position, an overlay appearing)?
  Confirmed while sizing this task: nothing in `src/app.rs` currently
  sets the terminal title (`\x1b]0;…\x07` or similar) or rings the
  terminal bell (`\x07`) on any state change — the only OSC sequences
  in the whole codebase are the existing cursor-shape ones
  (`src/app.rs:7397-7405`). Primarily a research/documentation
  deliverable, not a redesign: produce a written audit
  (`docs/accessibility/index.md`, new) covering (1) what a well-behaved
  TUI *can* signal a screen reader through (terminal title updates,
  the bell, nothing else reliably — most screen readers read visible
  text changes, not arbitrary TUI redraws) and (2) which of Vix's own
  state changes (a long-running `Project → Compile`/`Run`/`Test`
  finishing, a modal-editing mode switch, a blocking prompt opening)
  would benefit from one of those two channels. Ship whichever of those
  turn out to be clearly safe, low-risk wins (e.g. a terminal-title
  update or bell on a background command's completion, gated behind a
  setting so it's opt-in and never surprises someone who didn't ask for
...
- **Collaborative editing.** Multiple people editing the same buffer
  ...
  planned.
- ~~**Interactive 3-way merge conflict resolver.**~~ Promoted to
  **T556** 2026-09-22 (Run I) — see its own entry above.
- ~~**`vix --doctor`.**~~ Promoted to **T553** 2026-09-22 (Run I) — see
  its own entry above.
- ~~**Settings/profile export-import.**~~ Promoted to **T555**
  2026-09-22 (Run I) — see its own entry above.
- ~~**Accessibility audit for screen readers.**~~ Promoted to **T557**
  2026-09-22 (Run I) — see its own entry above.
- ~~**SBOM generation.**~~ Promoted to **T554** 2026-09-22 (Run I) —
...
**Every run and every group above is complete.** Run H (self-audit,
T506–T552) finished 2026-09-22, closing every task the two self-audit
passes turned up, T547 last of all (see its own entry — five real
CI-discovered rounds to get genuinely right on Windows).

10. **Run I (promoted from the Ideas backlog):** T553–T557, any order,
    one branch each. Started 2026-09-22.

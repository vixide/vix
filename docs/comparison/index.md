...
- A **bottom dock** for logs, output, and data (Run Command, project search).
...
- The default keyway (**Apple**) is not modal. Keyboard shortcuts follow typical
  macOS and Windows conventions — for example, `Ctrl-C` for Copy.
- Power users who prefer modal editing can opt into the **Vim keyway** via
  **View → Keyway…**. See [keyways](../keyways/index.md).

## Easier for novices than emacs

Emacs relies on chorded commands and its own vocabulary. Vix is easier because:

- The default keyway (**Apple**) uses macOS/Windows shortcuts — for example,
  `Ctrl-C` for Copy.
...
to Vim or Emacs keyways when they want them.

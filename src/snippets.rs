...
//! [`crate::snippet_tool::parse`]; this module handles the files and the merge.
...
/// The built-in snippets ([`crate::snippet_tool::SNIPPETS`]) as [`Snippet`]s with
/// [`Scope::Bundled`]. They have no expansion prefix (picker-only).
#[must_use]
pub fn bundled() -> Vec<Snippet> {
    crate::snippet_tool::SNIPPETS
        .iter()
        .map(|s| Snippet {
